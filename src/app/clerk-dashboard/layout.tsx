...
function ClerkAcademicYearSelect() {
  const { selectedAcademicYear, setSelectedAcademicYear, availableAcademicYears } =
    useAcademicYear();
  return (
    <div className="flex items-center gap-2">
      <Label htmlFor="clerk-academic-year" className="sr-only">
        Academic Year
      </Label>
      <Select value={selectedAcademicYear} onValueChange={setSelectedAcademicYear}>
        <SelectTrigger id="clerk-academic-year" className="w-[140px] sm:w-[160px]">
...
const navItems = [
  { href: '/clerk-dashboard', icon: LayoutDashboard, label: 'Dashboard' },
  { href: '/clerk-dashboard/teachers', icon: Users, label: 'Teachers' },
  { href: '/clerk-dashboard/admission', icon: UserPlus, label: 'Admission' },
  { href: '/clerk-dashboard/students', icon: Users, label: 'Students' },
...
export default function ClerkDashboardLayout({
  ...
  const principal = useCurrentPrincipal(); // Resolves current clerk
      ...
      <Sidebar>
        ...
        <SidebarFooter className="border-t border-sidebar-border">
          <SidebarMenu>
            ...
            <SidebarMenuItem>
              <Link href="/clerk-dashboard/profile">
                <SidebarMenuButton
                  className="justify-start gap-3"
                  isActive={pathname === '/clerk-dashboard/profile'}
                >
                  {profileAvatarUrl && (
                    <Image
                      src={profileAvatarUrl}
                      alt={principal?.name ?? 'Clerk'}
                      width={28}
                      height={28}
