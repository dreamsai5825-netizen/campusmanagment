...
import type { Complaint, Teacher, Student } from '@/lib/types';
...
export default function ComplaintsPage() {
  ...
  useEffect(() => {
    ...
    const unsubStudents = onSnapshot(
      ...
      (snapshot) => {
        setStudents(
          snapshot.docs.map((d) => ({ ...d.data(), id: d.id } as Student))
        );
      }
    );
    ...
    // Load teachers for coordinator assignment
  ...
  const filteredComplaints = complaints.filter((complaint) => {
    ...
    if (!complaint.studentId) return true;
    return yearStudentIds.has(complaint.studentId);
  });
  ...
  const handleAssignCoordinator = async (coordinatorId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Coordinator has been assigned to this complaint.',
      });
    } catch (error) {
      toast({
        variant: 'destructive',
    ...
    <div className="flex flex-col gap-6 sm:gap-8">
      <div className="min-w-0">
        ...
        <p className="text-muted-foreground text-sm sm:text-base">
          Complaints from students in {selectedAcademicYear}.
        </p>
      </div>
              ...
              >
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <StatusIcon className={`h-5 w-5 mt-1 ${statusColors[complaint.status as ComplaintStatus]}`} />
                  </div>
                </CardHeader>
              ...
              <div className="space-y-4 py-4">
                ...
                {selectedComplaint.status === 'in-progress' && (
                  <div>
                    ...
                    <Select value={assignedCoordinator} onValueChange={handleAssignCoordinator}>
                      <SelectTrigger id="coordinator" className="w-full mt-2">
