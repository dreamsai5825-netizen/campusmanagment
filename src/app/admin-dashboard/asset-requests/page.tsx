...
export default function PrincipalAssetRequestsPage() {
  ...
  useEffect(() => {
    if (!principal?.id || !principal?.collegeId) return;

    const q = query(
      collection(db, 'asset_requests'),
      where('collegeId', '==', principal.collegeId),
      where('requestedById', '==', principal.id)
    );
    ...
    return () => unsubscribe();
  }, [principal?.id, principal?.collegeId]);

  const handleCreateRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!principal?.collegeId || !principal?.id) return;
    ...
    try {
      await addDoc(collection(db, 'asset_requests'), {
        ...
        requestedBy: principal.name || 'Principal',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground text-sm sm:text-base">
            Request inventory, equipment, or lab resources from the College Administrator.
          </p>
        </div>
            ...
            <form onSubmit={handleCreateRequest}>
              ...
              <div className="space-y-4 py-4 text-sm">
                <div className="space-y-1.5">
                  ...
                  <Input 
                    placeholder="e.g. Projector, Whiteboard, CS Lab Laptop" 
                    value={assetName}
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="col-span-2 space-y-1.5">
                    ...
                    <Input 
                      placeholder="e.g. Science, IT, Administration" 
                      value={department}
